...
const HomePage: React.FC = () => {
  ...
  const handleImageClick = (images: string[], startIndex: number) => {
    if (images.length === 0) return; // Don't open for products with no images
      ...
      <main className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {loading ? (
                  ...
                  filteredProducts.map(product => <ProductCard key={product.id} product={product} onImageClick={handleImageClick} />)
              ) : (
